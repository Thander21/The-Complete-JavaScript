...
const dolphinsScores = totalScore(
  dolphins[0],
  dolphins[1],
  dolphins[2]
).toFixed(2);
...
if (dolphinsScores >= 100 || koalasScores >= 100) {
  if (dolphinsScores === koalasScores) {
    console.log(
      `Empate, não houve campeão a pontução é igual: ${dolphinsScores} = ${koalasScores}`
    );
  ...
  console.log(
    `Não houve campeão, nenhum pontuação: ${dolphinsScores}, ${koalasScores} é maior que 100 `
  );
